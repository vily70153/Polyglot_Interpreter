...
impl Logging { pub fn get_folder(&self) -> Option<std::path::PathBuf> { Some(std::path::PathBuf::from(&self.folder)) } pub fn ensure_log_dir(path: &std::path::PathBuf) { std::fs::create_dir_all(path) .expect("Failed to create log directory"); } }